class Employee:
    def __init__(self, name, age: int, work_experience: int, programming_languages=None, positions=None, programmer_level = None, salary=0, role=None):
        if age < 18:
    ...
    def get_info(self):
        ...
        if self.__programming_languages:
            languages = self.__programming_languages
        else:
            languages = 'Не указан'
        if self.__positions:
        ...
        level = self.__programmer_level if self.__programmer_level else 'Не указан'
